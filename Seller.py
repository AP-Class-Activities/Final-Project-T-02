...
class Seller:
    ...
    def profit(self, start_date, end_date):
        ...
        for purchase in self.sells:
            ...
            if start_date < file_date < end_date:
                sum_profits += 0.8 * purchase[-3] * purchase[-2]
        return sum_profits
    ...
    def products(self):
        self.__products = {}
        for product in self.__store.products:
            for seller in product.sellers:
                if seller.seller_id == self.__seller_id:
                    self.__products[product] = product.stock(seller)
        return self.__products

    @property
    def rating(self):
        try:
            return round(self.__rating[1]/self.__rating[0], 1)
